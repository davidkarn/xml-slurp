			...
			(reduce append '()
				(map (lambda (r) 
					   ...
					   (if (equal? #t (cdr r))
					       ...
					       (list (second (assoc (car r) curl-options)) (cdr r)))))
				     alis))))
	 (execute (lambda ()
		    (set! referer (cdr (assoc 'url request)))
		    (call-with-values 
			(lambda () (process (arglist->cmdline (append (list "curl") (options-lis request) (options-lis options)))))
		      (lambda (in out id)
    ...
    (set! this
	  (lambda (cmd . args)
	    (cond ((equal? cmd 'set)
		  ...
		  ((equal? cmd 'get)
		   (begin (set! request (list (cons 'url (car args)) (cons 'referer referer)))
			  (execute)))
		  ((equal? cmd 'post)
		   (begin (set! request (append 
					 (if (string? (cadr args))
					     (list (cons 'post-data (cadr args)))
					     (map (lambda (r) 
						    (cons 'data-urlencode (s+ (->string (car r)) "="
									      (->string (if (list? r)
											    (second r)
											    (cdr r))))))
						  (cadr args)))
		  ...
		  ((equal? cmd 'store-cookies)
		   (let ((cj (if (not (empty? args)) (car args) (create-temporary-file))))
		     (set! options (alist-update options 'cookie-jar cj))
		     (set! options (alist-update options 'cookie cj))
		     (set! cookie-jar cj)
		     cj))
		  ((equal? cmd 'catch-headers)
		   (let ((hf (if (not (empty? args)) (car args) (create-temporary-file))))
		     (set! options (alist-update options 'dump-header hf))
		     (set! header-dump hf)
		     hf))
		  ((equal? cmd 'trace)
		   (let ((tf (if (not (empty? args)) (car args) (create-temporary-file))))
